public class AuthorSignature {
    ...
    private final double avgWordLength, differentWordRatio, hapaxRatio, avgWordsPerSentence, avgPhrasesPerSentence;
    ...
    public double euclideanDistanceTo(AuthorSignature a){
        double[] unknownSet = a.getSignatureSet();
        double[] thisSet = this.getSignatureSet();
        double sum = 0.0;
        for (int i = 0; i < 5; i++) {
            sum += Math.pow(thisSet[i] - unknownSet[i], 2) * WEIGHT[i];
        }
